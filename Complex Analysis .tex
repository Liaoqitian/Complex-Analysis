...
\textbf{Finding branches}: At this point, it suffices to study paths of the form $\gamma(t) = z_0 + re^{it}$ for $t \in (-\pi, \pi)$, and see if $f(\gamma(t)) = f(\gamma(t + 2\pi))$ holds for all $t$. \\
...
\subsubsection{Complications with $\conj{z}$}

Consider the following example: \\
Let $g(z) = \Re(z) = \frac{z + \conj{z}}{2}$, which is a linear combination of continuous functions. \\
Assume $h \in \mathbb{R}$, $\frac{g(z + ih) - g(z)}{h} = \frac{\Re(z) - \Re(z)}{h} \to 0$ as $h \to 0$ \\
Compare this with $\frac{g(z + ih) - g(z)}{h} = \frac{\Re(z) + h -\Re(z)}{h} = 1 \to 1$ as $h \to 0$. \\
...
\textbf{Proof}: \\
Suppose $f$ is holomorphic on $\Omega$. Let $z_0 \in \Omega^{\ast}$ and $z_n \in \Omega^{\ast}$ for all $n$ and $z_n \to z$. Then $\conj{z_n} \to \conj{z_0}$ in $\Omega$ and for $\epsilon > 0$, there is a $N \in \mathbb{N}$ such that for $n > N$. 
...
\subsubsection{Properties of the Cauchy Riemann Equations} 
...
\subsubsection{Sufficiency of the Cauchy Riemann Equations to $f'$}
\textbf{Follow-Up Question}: The Cauchy Riemann Equations do a great job showing $f'$ does not exist. But what about it being sufficient for $f'$? \\
\textbf{Claim}: Satisfying the Cauchy Riemann Equations at $z_0$ implies that $f'$ exists at $z_0$. \\
\textbf{Proof}:\\
...
\[ \begin{cases} 
   ...
   \end{cases}
\]
\subsubsection{Relationship with the Cauchy Riemann Equations}
...
\newline
\textbf{Definition: Harmonic Conjugate} \\
...
\subsubsection{Conditions of Conformal Functions}
We say $f$ is a conformal map at $z_0$ if the following hold: \\
\circled{1} $f$ is $\mathbb{R}^2$-differentiable at $z_0$ \\
\circled{2} $\abs{\mbox{Jf}} \neq 0$ \\
\circled{3} $f$ preserves the oriented angle $\theta$, between $\gamma_1$ and $\gamma_2$ and $\tau_1$ and $\tau_2$ at $z_0$ and $f(z_0)$. 
\subsubsection{$e^z$ is conformal}
Now let us take a closer look at $e^z$ and figure out why it is conformal. \\
\circled{1} holds apparently. \\
\circled{2} $f(z) = e^z = e^x\cos(y) + ie^x\sin(y)$, then 
\begin{equation*}
J(f(x, y)) = 
\begin{bmatrix}
 e^x\cos(y) & -e^x\sin(y) \\
...
\circled{3} Now we have shown in the previous part that Jf is the product of a dilation matrix, $e^xI$, and a rotation matrix, which means $f$ preserves the angles between $\gamma_1'$ and $\gamma_2'$. Their image under $f$: \\
...
\circled{1} Composition of a finite number of 
\[ \begin{cases} 
   ...
   \end{cases}
\]
\circled{2} Conformal, $f$ is holomorphic away from $z = -\frac{d}{c}$, $f' \neq 0$, and f is one-to-one. \\
\circled{3} Maps circles/lines to either lines or circles, "lines are circles of $\infty$-radius in $\mathbb{C}$ or $\mathbb{C} \cup \{\infty\}$. \\
...
\circled{4} $f$ can be identified by 
\begin{equation*}
A = 
\begin{bmatrix}
 a & b \\
...
\circled{5} We can conformally map 3 points in $\mathbb{C} \cup \{\infty\}$ to any 3 points in $\mathbb{C} \cup \{\infty\}$. \\
...
\]
then 
$T(z) = (z, z_0, z_1, z_2) = \frac{(z -z_0)(z_1 - z_2)}{(z - z_2)(z_1 - z_0)}$ is called the cross-ratio of $z$, $z_0$, $z_1$, and $z_2$. \\
Special cases: 
\[ \begin{cases} 
