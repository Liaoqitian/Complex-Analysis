...
\begin{titlepage}
	...
	\line(1, 0){400}\\
	[0.25in]
	\huge{\bfseries Introduction to Complex Analysis} \\
	[2mm]
	\line(1, 0){300} \\
	[1.5cm]
...
\section*{Preface}
My name is Qitian Liao. I was born in Shanghai, China. I recently graduated from University of California, Berkeley on May $\nth{16}$, 2020, with a double degree in Computer Science and Applied Mathematics. With Corona Virus still rampaging and many BLM activities, our society has sunk into chaos. As I had to spend most of my time at home, there is no better time to reflect upon my undergraduate life. \\
\newline
I took Math 185 - Introduction to Complex Analysis with Professor Ryan Hass during my Junior year and it is definitely one of the most challenging classes I have had. I have a compulsion to get A's for all of my classes. Therefore, I have seriously considered late dropping the course right before the final in order to maintain my 4.0. I know this might sound as a joke to many of you, but this has truly happened to me. Thankfully, my friends and my professor persuaded me to hang in there till the end, and the results were not disappointing. But nevertheless, the class reminds me that there is barely anything I could not achieve as long as I put my mind into it. \\
\newline
This thought motivates me to write this book. On one hand, it helps me review what I know about Complex Analysis. On the other hand, this book is like no other on the market right now. It has contents easy to read, with examples easy to understand, and generally, I find it useful for people just started learning Complex Analysis and do not wish for something too complicated just yet. It is my first time writing a book, so I apologize beforehand if anything does not meet your satisfaction. Thank you for reading, I put in a great amount of effort into this. 
\thispagestyle{empty}
...
Let the power series centered at $z_0$ be
...
\newline
Before we move on to more specifics about the radius of convergence, a brief review of terminology is in order. A reminder about upper and lower limits. Let 
\begin{align*}
&\text{Largest lower bound: } \lim_{n \to \infty} \inf|a_n| = L \\ 
&\text{Smallest upper bound: } \lim_{n \to \infty} \sup|a_n| = U \\
\Longrightarrow \hspace{2mm}&L = \sup_{n \geqslant 0}(\inf_{k \geqslant n}|a_k|) = \lim_{n \to \infty} (\inf \{ |a_n|, |a_{n + 1}|, \cdots \}) \\
...
$$ |S_n(z)| = \sum_{k = 0}^n \abs[\big]{a_kz^k}$$
...
\newline
\textbf{Example.} Determine the radius of convergence of the following series. 
$$f(z) = \sum_{n = 1}^{\infty} \frac{z^n}{n!} $$
Then in this case, $a_n = \frac{1}{n!}$
\begin{align*}
&\lim_{n \to \infty} \abs*{\frac{a_{n + 1}}{a_n}} = \lim_{n \to \infty} \frac{n!}{(n+1)!} = \lim_{n \to \infty} \frac{1}{n + 1} = 0 = \frac{1}{R}, R = \infty \\
\Longrightarrow \hspace{2mm} &f(z) = \sum_{n = 1}^{\infty} \frac{z^n}{n!} = e^z - 1 \text{ converges for all } z \in \mathbb{C}. 
\end{align*}
\newpage
\textbf{Example.} Determine the radius of convergence of the following series. 
...
$$ \sum_{n = 0} ^ {\infty} (-1)^n \frac{z^{2n}}{3^{2n}} = \frac{1}{1 - \left(-\frac{z}{3}\right)^2} = \frac{1}{1 + \frac{z^2}{9}} = \frac{9}{9 + z^2} = \frac{d}{dz}3\arctan\left(\frac{z}{3}\right)$$
Most of the power series with reals still work here in complex. 
$$\sin(z) = \sum_{n = 0}^{\infty} \frac{(-1)^nz^{2n + 1}}{(2n + 1)!}, z \in \mathbb{C}$$
However, because $\sqrt{z}$ is not differentiable at $z = 0$. 
$$ \sqrt{z} \neq \sum_{n = 0}^{\infty} a_nz^n $$ 
\subsection{Derivative of Power Series}
Now let us prove the claim we made about the derivative of a power series.\\ 
\textbf{Claim.} The derivative of a power series is given as below. 
$$f'(z) = \sum_{n = 0}^{\infty}na_nz^{n - 1}, |z| < R$$ 
\textbf{Proof.} Define $g(z)$ as 
$$g(z) = \sum_{n = 0}^{\infty} a_n\cdot nz^{n - 1}$$ 
and if 
$$ L = \frac{1}{R} = \lim_{n \to \infty}\sup |a_n|^{\frac{1}{n}} $$
$$ \Longrightarrow \lim_{n \to \infty} \sup |na_n|^{\frac{1}{n}} = \lim_{n \to \infty}\sup |n|^{\frac{1}{n}}|a_n|^{\frac{1}{n}} = \lim_{n \to \infty}\sup|n|^{\frac{1}{n}}\lim_{n \to \infty} \sup |a_n|^{\frac{1}{n}}$$
...
$$f(z) = \sum_{n = 0}^N a_nz^n + \longsum[10]_{n = N + 1}^{\infty} a_nz^n = S_n(z) + E_n(z)$$ 
where $|z| < r < R$, 
...
\begin{align*}
&\frac{S_N(z + h) - S_N(z)}{h} - S_N'(z) \text{ derivative of a polynomial, }  \\
&S_N'(z) - g(z), S_N'(z) \to g(z) \text{ for } N > M \text{ for some } M, \\
&\frac{E_N(z + h) - E_N(z)}{h}, \text{ this is more technical and we will investigate more} \\
&\abs*{\frac{E_N(z + h) - E_N(z)}{h}} \leqslant \longsum[10]_{n = N + 1}^{\infty} |a_n|\abs*{\frac{(z + h)^n - z^n}{h}} 
\end{align*}
This means we need a bound on $(z + h)^n - z^n$. Remember 
\begin{align*}
&1 + x + x^2 + \cdots + x^{n - 1} = \frac{1 - x^n}{1 - x} \\
\Longrightarrow \hspace{2mm} &(1 - x) (1 + x + x^2 + \cdots + x^{n - 1}) = 1 - x^n
\end{align*}
If we let $x = \frac{b}{a}$, then we have a more generalized formula, 
$$a^n - b^n = (a - b)(a^{n - 1} + a^{n - 2}b + \cdots + ab^{n - 2} + b^{n - 1})$$
Apply the formula above to $a = z + h$ and $b = z$, 
$$|(z + h)^n - z^n| = |h|\abs[\big]{(z+h)^{n - 1} + (z+h)^{n - 2}z + \cdots + z^{n - 1}}$$
We have n terms of the form 
$$(z + h)^{n - 1 - k} \cdot z^k$$
and if $h$ is small, $|z+h| \leqslant r < R$, then 
$$\abs[\big]{(z + h)^{n - 1 - k} \cdot z^k} \leqslant r^{n -1} \leqslant R^{n - 1}$$
Therefore, 
$$ |a_n|\frac{|(z+h)^n - z^n|}{|h|} \leqslant |a_n|\frac{|h|nR^{n - 1}}{|h|} = |a_n|nR^{n -1} $$
Hence, we have a bound on
$$\abs*{\frac{E_N(z+h) - E_N(h)}{h}} \leqslant \longsum[10]_{n = N + 1}^{\infty} |a_n|nr^{n - 1}$$
...
or in other words, $f'(z) = g(z)$. \\
\newline
\textbf{Proposition.} \\
Power series functions are holomorphic in $|z - z_0| < R$ and are naturally $\infty$-differentiable, i.e. $\mathbb{C}^\infty$. \\
\newpage 
Integrating term by term is allowed provided we have uniform convergence. Furthermore, the partial sums are polynomials and holomorphic, and so the limiting function is holomorphic. 
$$\sum_{n = 0}^{\infty}nz^n = z\sum_{n = 0}^{\infty}nz^{n - 1} = z\frac{d}{dz}\frac{1}{1-z} = \frac{z}{(1-z)^2}, |z| < 1$$
...
\begin{figure}[H]
\includegraphics[scale = 0.2]{17_1}
\centering
\end{figure} 
...
\Longrightarrow f(z) &= \frac{1}{2\pi i}\oint_C \frac{f(w)}{w - z} \,dw \\
&= \frac{1}{2\pi i} \oint_C \frac{1}{w} \sum_{n = 0}^{\infty} f(w) \left(\frac{z}{w} \right)^n \,dw \\
&= \frac{1}{2\pi i}\sum_{n = 0}^{\infty} \oint_C f(w)\frac{1}{w}\left(\frac{z}{w}\right)^n \,dw \\
&= \sum_{n = 0}^{\infty} \frac{1}{2\pi i}\oint_C \frac{f(w)}{w^{n + 1}}\,dw z^n \\
&= \sum_{n = 0}^{\infty}a_nz^n \\
\text{where } a_n &= \frac{1}{2\pi i}\oint_C\frac{f(w)}{w^{n + 1}}\, dw = \frac{f^{(n)}(0)}{n!} 
\end{align*}
Notice that because of uniform convergence we can switch the places of summation and integral.\\
\newline
\textbf{Theorem.} \\
...
\textbf{Proof.} Let the power series $f(z)$ be 
$$f(z) = \sum_{n = 0}^{\infty} a_n(z-z_0)^n$$
...
$$f(z) = a_m(z - z_0)^m + \longsum[10]_{n = m + 1}^{\infty} a_n(z - z_0)^n = a_m(z - z_0)^m\Bigg(1+\underbrace{\longsum[10]_{n = m + 1}^{\infty}\frac{a_n}{a_m}\frac{(z - z_0)^n}{(z - z_0)^m}}_{g(z)}\Bigg) = a_m(z-z_0)^m(1+g(z))$$
so $g(z) \to 0$ as $z \to z_0$. The function $g(z)$ exists and is holomorphic and continuous since $m < n$ in the sum and $a_m \neq 0$.
Consider our sequence $z_k$ that converges to $z_0$, then $a_m(z_k - z_0)^m \neq 0$ since $z_k \neq z_0$ is clearly not a root of $(z - z_0)^k$ and $1 + g(z_k) \to 1 + 0 = 1$
...
\end{enumerate}
So by the Uniqueness Theorem, $f(z)$ is defined on its domain by its behavior near $z_0$. \\
\newline
\textbf{Example.}
\begin{align*}
f(z) &= 
...
Let $f(z) = \sin \left(\frac{1}{z}\right)$, $z_n = \frac{1}{n\pi}$, so that $z_n \to 0$. 
...
\begin{figure}[H]
\includegraphics[scale = 0.2]{18_1}
\centering
\end{figure} 
...
Let $z \in L$ and consider a ball about $z$. Let $C$ be a closed loop in the ball. 
\begin{figure}[H]
\includegraphics[scale = 0.25]{18_2}
\centering
\end{figure} 
If $C$ does not cross $L$, then $\oint_C f(z) \, dz = 0$ by Cauchy's Theorem. If $C$ does pass through $L$, we can break up the curve as follows: 
\begin{figure}[H]
\includegraphics[scale = 0.25]{18_3}
\centering
\end{figure} 
...
\begin{figure}[H]
\includegraphics[scale = 0.2]{18_4}
\centering
\end{figure} 
\textbf{Note.} To do this extension in a continuous manner is trivial. We are extending $f$ to a holomorphic function below $\mathbb{R}$-axis by reflection. The set $\Omega \cup L \cup \Omega^\star$ is symmetric about the $\mathbb{R}$-axis. \\
...
\newline
\textbf{Corollary.} 
If $f$ is holomorphic in a region symmetric with respect to $\mathbb{R}$-axis and $f(z)$ is $\mathbb{R}$ for $z \in \mathbb{R}$, then by the Uniqueness Theorem, $f(z) = \conj{f(\conj{z})}$.\\
\newline
\textbf{Example.} By the corollary above, for all $z$, $\cos(z) = \conj{\cos(\conj{z})}$.\\ 
\newline
...
For $z \in A$ and $f$ holomorphic in $A = \{z|r_1 < |z - z_0| < r_2\}$, we have 
\begin{align*}
...
\end{enumerate} 
\textbf{Example.} Expand the following function into a Laurent Series. 
$$\frac{z}{z^2 + 1}, \, z_0 = i$$ 
...
If $n = 1$, then $f$ has a \textit{simple root} at $z_0$. \\
\newline
\textbf{Pole.} \\
...
$$f(z) = \longsum[4]_{k = -n}^{\infty} a_k(z - z_0)^k = \frac{b_n}{(z - z_0)^n} + \frac{b_{n - 1}}{(z - z_0)^{n - 1}} + \cdots$$
for $n > 0$ and $b_n \neq 0$, then $f$ has \textit{a pole of order $n$} at $z_0$. \\
If $b_n = 0$ for $n > 1$ and $|b_1| \neq 0$, then $f$ has \textit{a simple pole} at $z_0$. \\
\newline
\textbf{Essential Singularity.} \\
If the principal part of $f(z)$ has $\infty$-many terms in it at $z_0$, then $f$ has an \textit{essential singularity} or a \textit{non-removable pole} at $z_0$. On the other hand, order-n poles are removable. \\
\newline
...
\newline
\textbf{Example.} 
$\frac{1}{z}$ is meromorphic on $\mathbb{C}$. $\csc(z)$ is meromorphic on $\mathbb{C}$, but it has $\infty$-many poles, namely $z = k\pi$ for $k \in \mathbb{Z}$. \\
\newline
...
has no pole at $z = 0$, since $\sin(z)$ and $z$ have simple roots at 0. Therefore, $f(z)$ has an isolated, removable singularity at 0.
\newpage
...
$$Res(f, z_0) = b_1 = a_{-1}$$
\newline
Remember, for any closed simple curve about 0, we have 
$$
\oint_C z^k \, dz = 
\begin{cases}
	0, &\text{ if } k \neq -1 \\
...
$$
\begin{figure}[H]
\includegraphics[scale=0.25]{20_1} 
\centering
\end{figure}
...
\newline
\textbf{Example.} Suppose $f(z)$ is defined as 
...
\newline
\textbf{Example.} $\operatorname{Log}(z)$ has a non-isolated singularity at 0. Therefore, $Res(\operatorname{Log}(z), 0)$ does not exist. \\
\newline
\textbf{Empirical Rules.} 
\begin{enumerate}[leftmargin=*]
\item If $ f(z) = \sum_{n = -1}^{\infty} a_n (z-z_0)^n $, then $Res(f(z), z_0) = a_{-1} = b_1$, and $f$ has a simple pole at $z_0$.
...
(The limit exists and we have equality.) Conversely, if $ \lim_{z \to z_0} (z-z_0)f(z)$ exists, then $f$ has a simple pole at $z_0$ or $f$ is $\mathbb{C}$-differentiable at $z_0$ and 
...
Therefore, by \circled{\scriptsize3} $Res(f, 2) = g(2) = -\frac{4}{3}$. In this case, we do not need to compute the Laurent Series at $z_0 = 2$. \\
\newline
\textbf{Example.}
$$ f(z) = \frac{1}{\sin(z)}$$ 
...
$$\left(\frac{b_1}{z} + a_0 + a_1z + a_2z^2 + \cdots\right)\left(z - \frac{z^3}{6} + \frac{z^5}{5!} - \cdots\right) = 1 - \frac{z^2}{2} + \frac{z^4}{4!} - \cdots$$
After we equate the powers of $z$, we have
$$b_1 + a_0z + \left(-\frac{b_1}{6} + a_1\right)z^2 + \left(-\frac{a_0}{6} + a_2\right)z^3 + \left(\frac{b_1}{5} - \frac{a_1}{6} + a_3\right)z^4 = 1 - \frac{z^2}{2} + \frac{z^4}{4!}$$
...
\Longrightarrow \oint_Cf(z) \, dz &= \oint_{C_3 \cup C_6} f(z) \, dz = \oint_{C_3}f(z) \,dz + \oint_{C_6}f(z)\,dz = 2\pi i(Res(f, z_1) + Res(f, z_2)) 
\end{align*}
...
Suppose $ I = \int_{\mathbb{R}}f(x) \,dx$ converges absolutely. Then
\begin{align*}
&I = \lim_{R\to\infty}\int_{-R}^{R}f(x) \, dx = P.V.\int_{\mathbb{R}}f(x) \, dx \\
\text{where } &P.V.\int_{\mathbb{R}}f(x) \, dx = \lim_{R\to\infty}\int_{-R}^{R}f(x) \, dx  
\end{align*}
denotes the principal value of $ \int_{\mathbb{R}}f(x)\, dx$. 
However, without absolute convergence of $I$, this does not hold. \\
\newline
\textbf{Example.}
\begin{align*}
&\lim_{R \to \infty} \int_{-R} ^{R} \sin(x) \,dx = 0 \\
\text{but } \int_{\mathbb{R}}\sin(x) \, dx &\text{ does not exist, since } \int_{\mathbb{R}}|\sin(x)| \,dx = \infty
\end{align*}
...
\end{figure}
\newpage
\textbf{Example.} Suppose $ f(x) = \frac{1}{x^3}$ and we want to compute $I = P.V.\int_{-2}^{3}f(x) \, dx$. 
\begin{align*}
...
\end{align*}
We canceled out $-F(\varepsilon)$ and $F(-\varepsilon)$ since $F$ is even. We are ``ignoring'' the singularity by exploiting $\int_{-a}^{a}f(x) \, dx = 0$ for $f$ odd and piecewise-smooth. \\
\newline
\textbf{Lemma.} \\
If $|f(x)| \leqslant g(x)$ for all $x \in \mathbb{R}$ and $\int_{\mathbb{R}}g(x) \, dx$ converges, then $I = \int_{\mathbb{R}} f(x) \, dx$ converges absolutely. 
...
\Longrightarrow \hspace{2mm}&\frac{1}{i} \oint_{|z| = 1} \frac{dz}{z^2 + 5z + 1} = \frac{2\pi i}{i} \frac{1}{\left(z - \frac{1}{2}(-5 - \sqrt{21})\right)} \Bigg\rvert_{z = \frac{-5 + \sqrt{21}}{2}} = \frac{2\pi}{\sqrt{21}}
\end{align*}
\newline
...
\text{then } &\lim_{\varepsilon \to 0^+} \int_{C_\varepsilon}f(z) \, dz = \pi i \, Res(f, z_0)
\end{align*}
\begin{figure}[H]
\includegraphics[scale=0.2]{21_4} 
\centering
\end{figure} 
...
\int_Cf(z)\,dz &= \int_0^\pi f\left(z_0 + \varepsilon e^{it}\right)\varepsilon i e^{it }dt \\ 
...
If for $z$-large, 
$\displaystyle{|f(z)| \leqslant \frac{A}{|z|}}$ and $C_R$ is the curve parametrized by 
$$ \gamma(t) = Re^{it}, t \in [0, \pi]$$
then for $\alpha > 0$, 
$$ \lim_{R \to \infty}\int_{C_R}f(z) e^{i\alpha z}\, dz = 0$$ 
\newline
\newline
\textbf{Example.} Suppose we want to find 
...
\begin{figure}[H]
\includegraphics[scale=0.2]{21_5} 
\centering
\end{figure} 
...
\Longrightarrow \hspace{2mm}&P.V.\int_{\mathbb{R}}\frac{e^{ix}}{x} \, dx = \pi i \, Res(f, 0) = \pi i e^{iz} \bigg\rvert_{z = 0} = \pi i
\end{align*}
\newline
\textbf{Example.} Find 
$\displaystyle{I = \int_0^{\infty} \frac{\sqrt{x}}{x^2 + 1} \, dx}$
where $\displaystyle{\sqrt{x} > 0}$.
...
\end{enumerate}
\begin{figure}[H]
\includegraphics[scale=0.2]{21_6} 
\centering
\end{figure} 
...
\begin{figure}[H]
\includegraphics[scale=0.5]{21_7} 
\centering
\end{figure} 
...
\begin{figure}[H]
\includegraphics[scale=0.2]{22_1} 
\centering
\end{figure} 
...
\end{align*} 
Therefore, $\frac{f'}{f}$ has a simple pole and $Res\left(\frac{f'}{f}, z_0\right) = k$, which is also the order of the root at $z_0$ of $f$. \\
...
For any closed curve $\gamma$ in $\mathbb{C}$, its \textit{winding number (index)} about $z_0$ is 
...
\begin{figure}[H]
\includegraphics[scale = 0.25]{22_7}
\centering
\end{figure}
...
\begin{figure}[H]
\includegraphics[scale = 0.17]{22_8}
\centering
\end{figure}
\newpage
...
&= R^n
\end{align*}
On $\displaystyle{|z| = R}, |f(z)| = R^n \Rightarrow |h| < |f|$ for $|z| = R$. \\
